...
**Collision Resolution:** Stationarity is checked first — a player who holds position ([0,0]) beats any mover regardless of attributes, and two stationary players on the same cell do not collide. Otherwise: Speed → Strength → Football IQ → Hands → Defense wins ties. Winner stays active; loser is neutralized 1–3 ticks (duration by Strength; the loser's avg(Football IQ + Hands) shortens recovery). Design intent: positioning can beat attributes, and planting early on the square a play must come through controls its lanes — arrive, then stop; standing still is a choice, not a default. A short recovery duration is likewise a tool: a fast-recovering player can commit to the square of a stationary player knowing they will lose that collision (stationarity wins) but recover in time to still affect the play.
